...
serve(async (req) => {
  ...
  try {
    const { contentType, companyInfo, serviceId, locationId, prompt } = await req.json();
    ...
    const openAIResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini",
        messages: [
          { role: 'system', content: 'You are a Local SEO expert and experienced content writer specializing in creating high-quality, SEO-optimized content for business websites.' },
